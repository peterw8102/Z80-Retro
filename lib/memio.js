/* global setTimeout console setInterval clearInterval */
/** @module memio
...
function toBin6(val) {
  return ('000000000000000000000000'+val.toString(2)).substr(-24);
}
...
pins[15] = new Gpio(15, {mode: Gpio.INPUT, pullUpDown: Gpio.PUD_UP});
...
function writeByte(address, data, strobe=true) {
  ...
  const writeVal = (address << 8 | (data & 0xff));

  // console.log("Starting WRITE BYTE "+data.toString(16)+" TO: "+address.toString(16));
  return new Promise((resolve) => {
    // Sequence is:
    ...
    // Write the data
    var set =  mapValue(writeVal) & limitMask;
    var clr = (~set) & limitMask;

    // log("BITS. VALUE, SET, CLR");
    // log(toBin6(writeVal)+' - '+toHex6(writeVal));
    // log(toBin6(set)+' - '+toHex6(set));
    // log(toBin6(clr)+' - '+toHex6(clr));
    bank.set(set);
    bank.clear(clr);
    ...
    if (strobe) {
      ...
      // console.log("Starting write strobe");
      wait(1).then(() => {
        // console.log("Toggle write -> low");
...
/* And that BUSREQ is inactive (high) */
reqBit.digitalWrite(1);
...
function busRequest(req) {
  // console.log("REQUESTING BUS: ", req);
  if (req==false)
...
module.exports = {
  wait, readByte, writeByte, read, write, readBit, writeBit, ackBit, reqBit, bus, pins, bank, mapValue, requestBus, busRequest
};
